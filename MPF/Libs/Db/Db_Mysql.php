...
class Db_Mysql {
    ...
    /**
     * 定义返回结果集数组的模型,取值如下:
     * MYSQL_NUM   :使用数字作数组下标索引,下标从 0 开始
     * MYSQL_ASSOC :使用字段名作数组下标索引(推荐)
     * MYSQL_BOTH  :同时使用数字和字段名作数组下标索引
     */
    private $FETCH_MODE = MYSQL_ASSOC;
	...
	private function Connect($Host='',$User='',$Pass='',$DB='',$Port='') 
	{
	    ...
	    $con = 'mysql_connect';
	    if ($this->PConnect) $con = 'mysql_pconnect';
	    ...
	    if ($this->Charset != '') @mysql_query('SET NAMES '.$this->Charset,$this->LinkID);
	...
	{
		$this->Execute($sql);
		$row = @mysql_fetch_array($this->QueryID,$this->FETCH_MODE);
		return $row;
	}
	...
	public function fetchCol($sql)
	{
	    $result = array();
		$this->Execute($sql);
		while ($row = @mysql_fetch_array($this->QueryID,$this->FETCH_MODE))
		{
		    $result[] = current($row);
		}
		return $result;
	...
	{
	    $value = @mysql_real_escape_string($value,$this->LinkID);
	    return "'$value'";
	}
	...
	{
	    $resource = mysql_query('SET AUTOCOMMIT=0',$this->LinkID);
	    if (!$resource) return false;
	    $resource = @mysql_query('BEGIN',$this->LinkID);
	...
	{
	    $resource = @mysql_query('COMMIT',$this->LinkID);
	    if (!$resource) return false;
        else return true; 	    
	}
	...
	{
	    $resource = @mysql_query('ROLLBACK',$this->LinkID);
	    if (!$resource) return false;
        else return true; 	    
	}
