...
class SplitWord
{
	...
	function __construct($loaddic=true)
	{
		if($loaddic)
		{
			...
			//高级分词，预先载入词典以提分词高速度
			$dicfile = "http://van.demo.com/Index/word.csv";
			$fp = fopen($dicfile,'r');
			while($line = fgets($fp,64)){
				$ws   = explode(' ',$line);
				$ws[0] = iconv("UTF-8", "GBK", $ws[0]);
				$ws[1] = iconv("UTF-8", "GBK", $ws[1]);
				$this->RankDic[strlen($ws[0])][$ws[0]] = $ws[1];
			}
			//print_r($this->RankDic);
			fclose($fp);
			//print_r($this->RankDic);
	...
	function SplitRMM($str="",$tryNumName=true,$tryDiff=true){
		...
		//echo iconv("GBK", "UTF-8", $this->SourceString);
		//对特定文本进行分离
			...
			else
			{
				...
				else //正常短句
				{
					if(strlen($spwords[$i]) <= $this->SplitLen)
					{
						...
						//echo $i.' '.$spwords[$i].'<br>';
						//echo $i.' '.$this->RunRMM($spwords[$i],$tryNumName,$tryDiff).$spc.'<br>';
	...
	function pcnd($w) {
		if(strlen($w)>2) {
			...
			if(preg_match("/[^0-9:-]/", $w)) {
				// 特殊英文单词处理
				//if ($w=="apple" || $w=="montella") {
				//return $w;
				//} else {
				//if(strlen($w)!=2&&ord($w[0])>0x80) {
				if(strlen($w)!=2&&ord($w[0])>0x40) {
					//echo iconv("GBK", "UTF-8", $w);
					return $w;
				}
				//}
			}
	...
	// 输出所需标题数组
	//--------------------
	function PutTitle($array){
		// 输出[火箭] => 2 [19] => 1 [连胜] => 1这样的数组
		foreach($array as $k) {
			if ($this->pcnd($k)) {
				$keywords .= $k.'-'.$num."|";
				$key[$k] = $key[$k]+1;
		...
		// 输出Array ( [0] => Array ( [title] => 火箭 [num] => 2 ) [1] => Array ( [title] => 连胜 [num] => 1 )
		$i = 0;
		foreach($key as $m=>$n) {
			$key2[$i][0] = iconv("GBK", "UTF-8", $m);
			$key2[$i][1]   = $n*5;
			$i++;
		}
		usort($key2, create_function('$a,$b','return $b[1]-$a[1];'));
		return $key2;
	}
	...
	// 带词频分词函数 cp参数为是否带词频，默认为带
	function keyword($body, $title='', $num) {
		// 去除一些特珠符号
		...
		$body = str_replace("net", " ", $body);
		$body = str_replace("cn", " ", $body);
		...
		$body = str_replace("6", " ", $body);
		$body = str_replace("7", " ", $body);
		$body = str_replace("8", " ", $body);
		$body = str_replace("9", " ", $body);*/

		// 将标题和文章切开
		//		$titleindexs = explode(" ", trim($this->splitrmm($title.''.$title.''.$title.''.$title.''.$title)));
		//$titleindexs = explode(" ", trim($this->splitrmm($title.'-'.$title.'-'.$title.'-'.$title.'-'.$title)));
		$body = strip_tags($body.$title.'-'.$title.'-'.$title.'-'.$title.'-'.$title.'-'.$title.'-'.$title.'-'.$title.'-'.$title.'-'.$title);
		//echo iconv("GBK", "UTF-8", $body);
		$allindexs = explode(" ", trim($this->SplitRMM($body)));


		//if (is_array($titleindexs)) {
		//$keytitle = $this->PutTitle($titleindexs);
		//}
		//print_r($allindexs);
		if (is_array($allindexs)) {
			$keyall = $this->PutContent($allindexs);
		}
		$this->clear();
		// 给字符串加上斜线，避免数据库查询出问题
		$keywords = AddSlashes($keywords);
		// 去除头尾|
		//		if(substr($keywords,strlen($keywords)-1,strlen($keywords))=='|') {
